...
export const Study = Backbone.Model.extend({
    ...
    parse(d) {
        ...
        const biomes = data.relationships.biomes.data.map(util.getBiomeIconData);
        let relatedStudies;
        ...
        return {
            bioproject_url: NCBI_URL + 'bioproject/' + attr['bioproject'] + '/',
...
export function getKronaURL(analysisID, ssuOrLsu) {
    return API_URL + 'analysis/' + analysisID + '/krona' + ssuOrLsu;
}
...
export const Sample = Backbone.Model.extend({
    ...
    parse(d) {
        ...
        return {
            ...
            biosample_url: NCBI_URL + 'biosample/' + attr['biosample'] + '/',
            biome: biome,
            biome_icon: util.getBiomeIcon(biomeName),
            biome_name: util.formatLineage(biomeName),
...
export const Analysis = Backbone.Model.extend({
    url() {
        return API_URL + 'analysis/' + this.id;
    },
    parse(d) {
        const data = d.data !== undefined ? d.data : d;
        const attr = data.attributes;
        ...
        return {
            ...
            sample_url: util.subfolder + '/samples/' + sampleID,
            run_accession: runID,
            run_url: util.subfolder + '/runs/' + runID,
            analysis_accession: data['id'],
            analysis_url: util.subfolder + '/analysis/' + d['id'],
            experiment_type: attr['experiment-type'],
...
export const RunAnalyses = Backbone.Collection.extend({
    ...
    url() {
        return API_URL + 'runs/' + this.id + '/analysis';
    },
    parse(d) {
        const data = d.data !== undefined ? d.data : d;
...
export const StudyAnalyses = Backbone.Collection.extend({
    ...
    url() {
        return API_URL + 'studies/' + this.id + '/analysis';
    },
    parse(response) {
        let analyses = response.data;
        const samples = response.included.reduce(function(obj, x) {
...
export const Taxonomy = RunPipelineObject.extend({
    url() {
        return API_URL + 'analysis/' + this.id + '/taxonomy' + this.type;
    },
    fetch() {
...
export const InterproIden = RunPipelineObject.extend({
    url() {
        return API_URL + 'analysis/' + this.id + '/interpro-identifiers';
    }
});

export const GoSlim = RunPipelineObject.extend({
    url() {
        return API_URL + 'analysis/' + this.id + '/go-slim';
    }
});
...
export const AnalysisDownloads = Backbone.Model.extend({
    url() {
        return API_URL + 'analysis/' + this.id + '/downloads';
    },
    parse(response) {
...
export const QcChartData = Backbone.Model.extend({
    ...
    url() {
        return API_URL + 'analysis/' + this.id + '/' + this.type;
    }
});
