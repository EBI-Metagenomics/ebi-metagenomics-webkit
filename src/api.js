...
define(["backbone", "underscore", "./util"], function (
  ...
  let init = function (options) {
    ...
    const Study = Backbone.Model.extend({
      ...
      parse(d) {
        ...
        return {
          ...
          samples_url:
            subfolder + "/studies/" + attr["accession"] + "#samples-section",
    ...
    const StudyGeoCoordinates = Backbone.Model.extend({
      url() {
        return API_URL + "studies/" + this.id + "/geocoordinates?page_size=500";
      },
      fetch() {
    ...
    const SampleStudiesCollection = Backbone.Collection.extend({
      ...
      initialize(params) {
        this.url =
          API_URL + "samples/" + params["sample_accession"] + "/studies";
      },
      parse(response) {
        return response.data;
      },
    ...
    const Sample = Backbone.Model.extend({
      ...
      parse(d) {
        ...
        const biomeName =
          data.relationships.biome.data && data.relationships.biome.data.id;
        return {
    ...
    const Analysis = Backbone.Model.extend({
      ...
      parse(d) {
        ...
        const attr = data.attributes;
        let studyID =
          data.relationships.study.data && data.relationships.study.data.id;
        let sampleID =
          data.relationships.sample.data && data.relationships.sample.data.id;
        let runID =
          data.relationships.run.data && data.relationships.run.data.id;
